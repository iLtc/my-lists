...
│       ├── CardGroup.js            # Year-grouped card wrapper
...
Language switching and section switching are handled together in `NavBar.js` — switching language preserves the current section (books vs. games).
...
-   In **development** (`NODE_ENV=development`) images are served directly without the Cloudflare CDN prefix.
...
Do **not** commit the `build/` directory — it is generated by CI.

---

## Known Gotchas

-   **Pre-existing test failure**: The sole test in `src/App.test.js` searches for "learn react" text (default CRA boilerplate) but the app has been customized. `CI=true yarn test` will fail — this is a known issue, not a broken environment.
